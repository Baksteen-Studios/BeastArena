...
int EntityFactory::createWeapon(double x, double y) const{
    auto dst = std::unique_ptr<Rect>(new Rect{ 0, 0, 0, 0 });
    auto r = renderableFactory.createImage(graphicsPath + "weapons/pistol.png", (int)Layers::Foreground, std::move(dst));
    auto comps = std::make_unique<std::vector<std::unique_ptr<Component>>>();

    comps->push_back(std::make_unique<TransformComponent>(x, y, 22, 32));
